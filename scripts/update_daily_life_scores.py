...
AMENITY_COUNTS_CACHE_PATH = Path("data/reference/osm_daily_life_counts.csv")
QUERY_RADIUS_METERS = 8_000

DIAGNOSTIC_COLUMNS = list(DAILY_LIFE_CATEGORIES.keys())


def build_daily_life_query(latitude: float, longitude: float) -> str:
    return build_daily_life_batch_query([(latitude, longitude)])


def _daily_life_query_blocks(latitude: float, longitude: float) -> list[str]:
    radius = QUERY_RADIUS_METERS

    return [
        f"""(
  node(around:{radius},{latitude},{longitude})["shop"~"^(supermarket|convenience|greengrocer|bakery|butcher|deli|general)$"];
  node(around:{radius},{latitude},{longitude})["amenity"="marketplace"];
);
out count;""",
        f"""(
...
out count;""",
        f"""(
  node(around:{radius},{latitude},{longitude})["amenity"~"^(cafe|restaurant|fast_food)$"];
);
out count;""",
        f"""(
  node(around:{radius},{latitude},{longitude})["leisure"~"^(fitness_centre|sports_centre)$"];
  node(around:{radius},{latitude},{longitude})["amenity"="gym"];
);
out count;""",
        f"""(
...
out count;""",
        f"""(
  node(around:{radius},{latitude},{longitude})["amenity"~"^(bank|atm|post_office|library|community_centre|townhall)$"];
);
out count;""",
        f"""(
  node(around:{radius},{latitude},{longitude})["amenity"~"^(clinic|doctors|dentist)$"];
  node(around:{radius},{latitude},{longitude})["healthcare"~"^(clinic|doctor|dentist|pharmacy)$"];
);
out count;""",
    ]


def build_daily_life_batch_query(locations: list[tuple[float, float]]) -> str:
    blocks = ["[out:json][timeout:120];"]

    for latitude, longitude in locations:
        blocks.extend(_daily_life_query_blocks(latitude, longitude))
    ...
    for attempt in range(max_attempts):
        ...
        try:
            response = session.post(
                url,
                data={"data": query},
                headers={"User-Agent": "CityFitAI/0.1 daily-life-score-updater"},
                timeout=75,
            )
...
def fetch_daily_life_counts(
    ...
    query = build_daily_life_query(latitude, longitude)
    response = post_overpass_query(
        query,
        session=session,
    ...
    if len(count_elements) != len(DAILY_LIFE_CATEGORIES):
        raise ValueError(
            "Unexpected Overpass response: expected "
            f"{len(DAILY_LIFE_CATEGORIES)} count elements, got {len(count_elements)}"
        )
    ...
    for column, element in zip(DAILY_LIFE_CATEGORIES, count_elements):
        counts[column] = _count_from_overpass_element(element)

    return counts


def fetch_daily_life_counts_batch(
    city_rows: list,
    session: requests.Session | None = None,
) -> list[dict]:
    locations = [
    ...
    query = build_daily_life_batch_query(locations)
    response = post_overpass_query(query, session=session)
    payload = response.json()
    count_elements = [
        element for element in payload.get("elements", []) if element.get("type") == "count"
    ]
    expected_count = len(city_rows) * len(DAILY_LIFE_CATEGORIES)
    ...
    rows = []
    category_count = len(DAILY_LIFE_CATEGORIES)
    ...
    for row_index, city_row in enumerate(city_rows):
        ...
        for column, element in zip(DAILY_LIFE_CATEGORIES, row_counts):
            counts[column] = _count_from_overpass_element(element)
...
def load_cached_counts(cache_path: Path = AMENITY_COUNTS_CACHE_PATH) -> pd.DataFrame:
...
def collect_daily_life_counts(
    ...
    for batch_start in range(0, len(city_rows), batch_size):
        ...
        try:
            batch_counts = fetch_daily_life_counts_batch(
                batch_rows,
                session=session,
            )
        ...
        print(f"{min(batch_start + batch_size, len(city_rows))}/{len(city_rows)} daily amenity rows cached")
...
def update_lifestyle_daily_life_scores(
    ...
    counts_df = collect_daily_life_counts(
        lifestyle_df,
    ...
    scored_df = add_daily_life_scores(lifestyle_df, counts_df)
    ...
    if completed_count < total_count:
        print(
            f"Cached daily life counts for {completed_count}/{total_count} cities. "
    ...
    scored_df["data_quality"] = scored_df["data_quality"].replace(
        {
            "not_started": "partial_daily_life",
            "partial_airport": "partial_daily_life_airport",
        }
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Update daily life scores using OpenStreetMap amenity counts."
    )
...
def main() -> None:
    args = parse_args()
    scored_df = update_lifestyle_daily_life_scores(
        limit=args.limit,
    ...
    if cached_count < total_count:
        ...
        print(
            "Updated daily life scores for "
            f"{scored_df['daily_life_score'].notna().sum()} cities in "
            f"{LIFESTYLE_METRICS_PATH}."
        )

    print(
        scored_df[["city", "country", "daily_life_score"]]
        .sort_values("daily_life_score", ascending=False)
        .head(10)
