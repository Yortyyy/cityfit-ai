...
CULTURE_COUNTS_CACHE_PATH = Path("data/reference/osm_culture_counts.csv")
QUERY_RADIUS_METERS = 8_000

DIAGNOSTIC_COLUMNS = list(CULTURE_COMPONENTS.keys())
    ...
    for attempt in range(max_attempts):
        ...
        try:
            response = session.post(
                url,
                data={"data": query},
                headers={"User-Agent": "CityFitAI/0.1 culture-score-updater"},
                timeout=75,
            )
...
def build_culture_query(latitude: float, longitude: float) -> str:
    return build_culture_batch_query([(latitude, longitude)])


def _culture_query_blocks(latitude: float, longitude: float) -> list[str]:
    radius = QUERY_RADIUS_METERS

    return [
        f"""(
  nwr(around:{radius},{latitude},{longitude})["tourism"="museum"];
  nwr(around:{radius},{latitude},{longitude})["amenity"="museum"];
);
out count;""",
        f"""(
  nwr(around:{radius},{latitude},{longitude})["amenity"~"^(theatre|arts_centre)$"];
);
out count;""",
        f"""(
  nwr(around:{radius},{latitude},{longitude})["tourism"~"^(gallery|artwork)$"];
);
out count;""",
        f"""(
...
out count;""",
  ...
  nwr(around:{radius},{latitude},{longitude})["tourism"~"^(attraction|viewpoint)$"]["historic"];
);
out count;""",
    ]


def build_culture_batch_query(locations: list[tuple[float, float]]) -> str:
    blocks = ["[out:json][timeout:120];"]

    for latitude, longitude in locations:
        blocks.extend(_culture_query_blocks(latitude, longitude))

    return "\n".join(blocks)


def fetch_culture_counts(
    city: str,
    country: str,
    latitude: float,
    longitude: float,
    ...
    query = build_culture_query(latitude, longitude)
    ...
    if len(count_elements) != len(CULTURE_COMPONENTS):
        raise ValueError(
            "Unexpected Overpass response: expected "
            f"{len(CULTURE_COMPONENTS)} count elements, got {len(count_elements)}"
        )
    ...
    for column, element in zip(CULTURE_COMPONENTS, count_elements):
        counts[column] = _count_from_overpass_element(element)

    return counts


def fetch_culture_counts_batch(
    city_rows: list,
    session: requests.Session | None = None,
) -> list[dict]:
    locations = [
    ...
    query = build_culture_batch_query(locations)
    response = post_overpass_query(query, session=session)
    payload = response.json()
    count_elements = [
        element for element in payload.get("elements", []) if element.get("type") == "count"
    ]
    expected_count = len(city_rows) * len(CULTURE_COMPONENTS)
    ...
    rows = []
    category_count = len(CULTURE_COMPONENTS)
    ...
    for row_index, city_row in enumerate(city_rows):
        ...
        for column, element in zip(CULTURE_COMPONENTS, row_counts):
            counts[column] = _count_from_overpass_element(element)
...
def load_cached_counts(cache_path: Path = CULTURE_COUNTS_CACHE_PATH) -> pd.DataFrame:
...
def collect_culture_counts(
    ...
    for batch_start in range(0, len(city_rows), batch_size):
        ...
        try:
            batch_counts = fetch_culture_counts_batch(
                batch_rows,
                session=session,
            )
        ...
        print(
            ...
            f"{len(city_rows)} culture rows cached"
        )
...
def update_lifestyle_culture_scores(
    ...
    counts_df = collect_culture_counts(
        lifestyle_df,
    ...
    scored_df = add_culture_scores(lifestyle_df, counts_df)
    ...
    if completed_count < total_count:
        print(
            f"Cached culture counts for {completed_count}/{total_count} cities. "
    ...
    scored_df["data_quality"] = scored_df["data_quality"].replace(
        {
            "not_started": "partial_culture",
            "partial_airport": "partial_culture_airport",
            "partial_daily_life_airport": "partial_daily_life_culture_airport",
            "partial_daily_life_food_airport": (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Update culture scores using OpenStreetMap culture counts."
    )
...
def main() -> None:
    args = parse_args()
    scored_df = update_lifestyle_culture_scores(
        limit=args.limit,
    ...
    if cached_count < total_count:
        ...
        print(
            "Updated culture scores for "
            f"{scored_df['culture_score'].notna().sum()} cities in "
            f"{LIFESTYLE_METRICS_PATH}."
        )

    print(
        scored_df[["city", "country", "culture_score"]]
        .sort_values("culture_score", ascending=False)
        .head(10)
